function [signalchange] = roi_signalchange_onesubject(ROIs, subject_stats_dir, event_duration)
...
% reminder
disp(['using ', event_string, [' to compute percent signal change.' ...
...
for i=1:nsess
    ...
    [p, ~, e] = fileparts(SPM.xY.VY(first_img_in_sess).fname);
    imgtype = e(2:end);
    imgdir = p;
...
for iroi=1:nroi % for each ROI
    ...
    display(['Processing ROI: ', ROI_list{iroi}])
    
    roi_obj = maroi(ROI_list{iroi});
    
    signalchange.roi_name{iroi} = label(roi_obj); % set ROI name
    ...
    for session=1:nsess
        for event=1:events_per_sess
            ...
            signalchange.data_roi_sess_event{iroi}{s}(e) = mean(pcs);
            ...
            signalchange.ts{iroi}{s}{e} = mean(timeseries, 2);
...
% tc(:, j) = this_tc / mean(block_means(E)) * 100;
% since block_means(E) returns an Nx1 matrix, N=no. of sessions
% end % for j
% tc_subj{i} = tc;
