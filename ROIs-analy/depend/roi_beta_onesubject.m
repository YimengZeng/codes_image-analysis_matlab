...
for session = 1:nsess
    ...
    for event = 1:events_per_sess
        ...
        if (strfind(SPM.Vbeta(e_s).descrip,'bf(1)')) % do not use derivatives bf(2)
            ...
            if(isempty(regexp(beta_img, '/'))) %#ok<*RGXP1>
        ...
        for iroi=1:nroi % for each ROI
            ...
            beta_average.roi_name{iroi} = label(roi_obj); % ROI names
            
            roi_betascores = getdata(roi_obj, beta_img, 'l'); % include NaNs
            roi_betascores(isnan(roi_betascores)) = [];
            beta_average.data_roi_sess_event{iroi}{s}(e) = ...
                mean(roi_betascores(find(roi_betascores < inf))); %#ok<*FNDSB>
