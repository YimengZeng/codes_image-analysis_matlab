...
for icon = 1:length(cond_num)
    ...
    for iroi = 1:length(roi_list)
        ...
        for isub = 1:length(sublist)
            ...
            img_sub  = fullfile(firlv_dir, yearID, sublist{isub,1},...
                'fMRI', 'Stats_spm12', task_name, ['Stats_spm12_', run_ppl], ...
                [img_type,'_000',cond_num{icon,1}(1),'.nii']);
            
            sub_img = spm_read_vols(spm_vol(img_sub));
            sub_vect = sub_img(mask(:)==1);
            
            grp_vect{isub,1} = sub_vect;
        end
        ...
        for isub = 1:length(sublist)
            ...
            for iisub = 1:length(sublist)
                if iisub == isub
                    ...
                    [rsa_r, rsa_p] = corr(grp_vect{isub,1}, grp_vect{iisub,1});
    ...
    save_name = ['res_rsa_intersub_intracond_',cond_num{icon,1}, '_', img_type, '.csv'];
