function fun_preproc_spm8_parallel(config_file, isess, isub)
warning('off') %#ok<*WNOFF>
currentdir = pwd;
disp('======================== Preprocessing Begining ========================');
fprintf('Current Directory: %s\n', currentdir);
fprintf('Script: %s\n', which('fun_preproc_spm8_parallel.m'));
...
yearID = ['20', isub(1:2)];
subt1_dir = fullfile(preproc_dir, yearID, isub, 'sMRI', 'Anatomy');
subt1_file = '';
...
totalsess_dir{sesscnt} = fullfile(preproc_dir, yearID, isub, ...
    'fMRI', isess);
...
if isempty(input_fliter)
    ...
    unix(sprintf('cp -af %s %s', fullfile(unnorm_dir, ['I.', data_type, '.gz']), ...
        temp_dir));
end

output_dir = fullfile(preproc_dir, yearID, isub, 'fMRI', isess, 'Smooth_spm8');
pfile_dir = fullfile(preproc_dir, yearID, isub, 'fMRI', isess, 'Pfiles');
