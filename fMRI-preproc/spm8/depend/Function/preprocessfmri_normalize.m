...
if ismember('g', PipeLine)
  ...
  if ~ismember('c', PipeLine)
    ...
    matlabbatch{1}.spm.spatial.normalise.estwrite.eoptions.template{1} = ...
      '/Users/haol/Dropbox/Toolbox/spm8/templates/EPI.nii,1';
  else
    matlabbatch{1}.spm.spatial.normalise.estwrite.subj.source{1} = SPGRFile;
    matlabbatch{1}.spm.spatial.normalise.estwrite.eoptions.template{1} = ...
      '/Users/haol/Dropbox/Toolbox/spm8/templates/T1.nii,1';
  end    
end

BatchFile = fullfile(OutputDir, 'Logs', ['batch_normalize_', WholePipeLine, '.mat']);
cd(OutputDir);
save(BatchFile, 'matlabbatch');
spm_jobman('run', BatchFile);
cd(CurrentDir);
unix(sprintf('ps2pdf13 %s %s', fullfile(OutputDir, 'spm_*.ps'), fullfile(OutputDir, 'Logs', ['normalize_spm8_', WholePipeLine, '.pdf'])));
