...
%% Movement Exclusion
if mvmntexclu == 1
    mconfigname = ['config_mvmntexclu_spm8_ALLSUB_',suffix,'.m'];
    ...
    for isess = 1:length(sesslist)
        ...
        for isub = 1:length(sublist_comm)
            ...
            rfile = fullfile(preproc_dir, yearID, sublist_comm{isub,1}, ...
                'fMRI', sesslist{isess}, 'Smooth_spm8/Logs', ...
                'realign_spm8_swcra.pdf');
            rfile_rename = ['realign_', sublist_comm{isub,1}, '.pdf'];
            wfile = fullfile(preproc_dir, yearID, sublist_comm{isub,1}, ...
                'fMRI', sesslist{isess}, 'Smooth_spm8/Logs', ...
                'normalize_spm8_swcra.pdf');
            wfile_rename = ['normalize_', sublist_comm{isub,1}, '.pdf'];
            
            unix(['cp ', rfile, ' ', fullfile(datacheck_dir, rfile_rename)]);
            unix(['cp ', wfile, ' ', fullfile(datacheck_dir, wfile_rename)]);
        end
    end
