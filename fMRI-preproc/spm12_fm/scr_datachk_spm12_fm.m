...
%% Movement Exclusion
if mvmntexclu == 1
    mconfigname = ['config_mvmntexclu_spm12_fm_ALLSUB_',suffix,'.m'];
...
if processlog  == 1
    ...
    for isess = 1:length(sesslist)
        ...
        for isub = 1:length(sublist_comm)
            ...
            vfile = fullfile(preproc_dir, yearID, sublist_comm{isub,1}, ...
                'fMRI', sesslist{isess}, 'Smooth_spm12/Logs', ...
                'calcuateVDM_spm12_swcra.pdf');
            vfile_rename = ['calcuateVDM_', sublist_comm{isub,1}, '.pdf'];
            ...
            unix(['cp ', vfile, ' ', fullfile(datacheck_dir, vfile_rename)]);
