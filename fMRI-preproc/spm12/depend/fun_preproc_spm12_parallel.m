...
function fun_preproc_spm12_parallel(fundir, t1dir, smthdir, slcorder, ...
...
% unix(sprintf('ps2pdf13 %s %s', fullfile(psfile_c.folder, psfile_c.name), pdffile_c));

% psfile_r = dir(fullfile(fundir, 'spm_*.ps'));
% pdffile_r = fullfile(fundir, ['realign_spm12_', runppl, '.pdf']);
% unix(sprintf('ps2pdf13 %s %s', fullfile(psfile_r.folder, psfile_r.name), pdffile_r));
